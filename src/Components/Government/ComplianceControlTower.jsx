            ...
            <div key={item.label} className="relative flex flex-col items-center group">
              ...
              <div className="flex flex-col-reverse gap-0.5 mb-3">
                {[...Array(totalSegments)].map((_, idx) => (
                  <div
                    key={idx}
                    className={`w-12 h-8 rounded-sm transition-all duration-300 ${
                      getSegmentColor(idx, litSegments, item.score)
                    } ${idx === 0 ? 'rounded-b-md' : ''} ${idx === totalSegments - 1 ? 'rounded-t-md' : ''} group-hover:scale-105`}
                  />
                ))}
              </div>

              {/* Score Badge */}
              <div className={`mt-2 rounded-full px-2 py-1 text-center ${
                ...
                <p className="text-xs font-bold">{item.score}%</p>
              </div>

              {/* Icon and Label */}
              <div className="mt-2 text-center">
                <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-slate-100 text-blue-700 mx-auto">
                  <Icon className="h-4 w-4" />
                </div>
                <p className="mt-1 text-[10px] font-bold text-slate-700">{item.label}</p>
                <p className="text-[8px] text-slate-400">{item.openCases} cases</p>
              </div>

              {/* Hover Tooltip */}
              <div className="absolute -top-24 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-20">
                <div className="rounded-lg bg-slate-800 px-3 py-2 text-center shadow-xl">
                  <p className="text-xs font-bold text-white">{item.label}</p>
                  <div className="mt-1 text-[9px] text-slate-300">
                    Compliance Score: {item.score}%
                  </div>
                  <div className="text-[9px] text-rose-300">
                    Open Cases: {item.openCases}
                  </div>
                </div>
                <div className="absolute left-1/2 -translate-x-1/2 -bottom-1 w-2 h-2 rotate-45 bg-slate-800" />
              </div>
            </div>
          );
        })}
