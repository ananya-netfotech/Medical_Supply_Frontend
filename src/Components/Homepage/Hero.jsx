...
export default function HeroSection() {
  ...
  <div className="relative rounded-[2rem] border border-blue-100 bg-white p-4 shadow-[0_30px_90px_rgba(15,23,42,0.12)] sm:p-6">
    <div className="relative overflow-hidden rounded-[1.5rem] bg-[#07111f] p-6 text-white">
      ...
      <div className="relative z-10">
        ...
        {/* Keeping the exactly identical four inner capability cards untouched */}
    ...
    {/* Highly Responsive Lifecycle Steps Sequence */}
    <div className="mt-5 grid grid-cols-1 gap-2.5 xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-5">
      {[
        "Register",
        "Manufacture",
        "Distribute",
        ...
        <div
          key={item}
          className="group relative overflow-hidden rounded-xl border border-blue-100 bg-blue-50/70 px-3 py-3.5 text-center text-sm font-semibold text-blue-900 shadow-xs transition-all duration-300 hover:-translate-y-0.5 hover:border-blue-200 hover:bg-white hover:shadow-md"
        >
          <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-blue-300 to-transparent" />
          
          {/* Layout fixes for mobile row presentation */}
          <div className="flex items-center justify-between xs:flex-col xs:justify-center">
            <div className="text-xs font-black text-blue-600/50 xs:mb-1">
              {String(index + 1).padStart(2, "0")}
            </div>
            <div className="text-slate-800 tracking-wide">
              {item}
            </div>
          </div>
        </div>
  ...
  <div className="pointer-events-none absolute -right-3 -top-3 h-6 w-6 rounded-full border border-blue-300/50 hidden sm:block" />
  <div className="pointer-events-none absolute -bottom-3 -left-3 h-6 w-6 rounded-full border border-blue-300/50 hidden sm:block" />
</div>
        </div>
      </div>
    </section>
